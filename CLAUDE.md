...
- **Stock firmware** (`xteink_app` 7.2.4, IDF 6.0.1 — the version every address in `docs/stock-firmware.md`
  refers to; the device runs **7.5.4** since 2026-09-07, re-located in `docs/stock-7.5.4.md`, and the stock
  tools read whichever the image says through `tools/stockver.d/`, see "The real device"): runs to Home with the device's NVS as dumped; its
...
tools/         x4emu shim, x4emu_mcp.py, mkflash.py, mksd.py, mkefuse.py, nvsedit.py, xic2png.py, stockver.py (+ stockver.d/: the per-version offsets the three stock tools read), stockdev.py, stockpatch.py, stockstrings.py, xtfont.py, xtapp.py, ghidra_stock.py (+ ghidra/), device.py, epdtrace.py, appdis.py, tcbwalk.py
tests/         pytest end-to-end (boot, home, touch, reader, battery, sleep, panel variants, stock, record/replay); fast: nvsedit, xic
apps/          Lua apps for the stock (status-typographic, status-panels, probe); docs/lua-apps.md
...
| `tools/stockver.py versions` / `identify IMAGE` | the per-version constant files `tools/stockver.d/*.json` (dev stub, patch sites, string packs, language count) that `stockdev.py`, `stockpatch.py` and `stockstrings.py` read: `versions` lists and validates them, `identify` says which one an image selects. The image's `esp_app_desc` picks the file (project + version) and its `app_elf_sha256` is a hard check — it survives our byte patches, the file hash does not. An unknown version is refused naming the known ones; a version missing one entry is refused for that entry alone (7.5.4's toast pack changed shape and is deliberately absent). Known today: 7.2.4, 7.5.4 (`tests/test_stockver.py`) |
| `tools/stockdev.py IMAGE [--check] [-o OUT]` | enable developer mode (Memory / Developer / **Screen Capture** in the pull-down light panel) in a stock image (7.2.4 or 7.5.4), bare app or 16 MB flash image (app0 at 0x10000): the predicate is a stub returning 0 (7.2.4 app VA 0x4233abe0, 7.5.4 0x4230dad4), so one byte becomes `movi.n a2,1` and the ESP checksum byte and appended SHA-256 are recomputed and re-verified; `--check` reports patched/unpatched/unknown (exit 0/1/2), any other firmware is refused (`tests/test_stockdev.py`; `tests/test_stock_capture.py` drives a patched copy to a capture that matches the panel) |
| `tools/stockstrings.py IMAGE list [--pack labels\|counts\|toasts\|all] [--lang en] [--grep T]` / `get GROUP` / `set GROUP LANG "text"` / `compact` / `restore --from ORIG` | read and relabel the stock UI string packs (7.2.4: labels 234 groups, counts 18, headerless toasts 688; 7.5.4: labels 269, counts 18, its u32 toast pack not yet loadable; four languages), bare app or 16 MB flash image: a text that fits is overwritten in place, a longer one appended into the free tail with its u16 repointed, checksum + SHA-256 recomputed; the stock packs have no free tail, `compact` frees 497 bytes (`tests/test_stockstrings.py`: a relabelled nav menu renders) |
| `tools/xtfont.py dump IN.xtf OUT.png [--text T\|--glyph CP]` / `build TTF --id ID --name N OUT_DIR` / `install SD.img PKG.xtfont [--direct]` | the stock's `.xtf` v2 system-font packages: parse and render (metrics `advance_x, advance_y, x_off, y_off`, pinned 0 px against the Home title), rasterise a TTF into the 20/24 px cells with manifest, `preview.xic` and the `.xtfont` ZIP, put it on a card (`--direct` rewrites `selection.config`); no `.hot.xtfp` needed; the header's two CRC-32 words (0x30 glyph data, 0x34 header) are written, so a converted TTF installs through Settings → System Font and the stock draws Home in it (`tests/test_xtfont.py`) |
| `tools/ghidra_stock.py analyze [--image PATH] [--version TAG] [--force]` / `xrefs ADDR` / `callers ADDR` / `func ADDR` / `decompile ADDR` | Ghidra over a stock app image (7.2.4 by default, `--version 7.5.4` for the other; Ghidra 12.1.3 + openjdk@21 from Homebrew; Ghidra ships the Xtensa processor): `analyze` (~1 min, idempotent) imports the image raw as `Xtensa:LE:32`, rebuilds one memory block per ESP segment at its load VA, labels the ROM from `images/rom/esp32s3_rev0_rom.nm`, exports `functions/xrefs/strings/calls.txt` to `images/ghidra/stock-<version>/` (24.8 k functions, 435 k xrefs, 96.5 k calls, 9.8 k strings); the queries are pure Python over those files (0.1 s), `decompile` a headless decompile of one function (~3 s; the SLEIGH misses a few opcodes — `appdis.py` is the fallback). GUI: `$(brew --prefix ghidra)/libexec/ghidraRun` on `images/ghidra/stock-7.2.4.gpr`, closed before `analyze`/`decompile` |
| `tools/stockpatch.py list IMAGE` / `apply\|revert IMAGE PATCH... [--check] [-o OUT]` | named byte patches on a stock image (7.2.4 or 7.5.4; the offsets come from `stockver.d/`), bare app or 16 MB flash (app0 at 0x10000): `developer-menu` (= stockdev.py), `hidden-menu-rows` (Preload List / Statistics), `lua-apps-row` (an extra nav row — "Lua Apps" in 7.2.4, **"Mini Apps"** in 7.5.4 — → the app list); each site's bytes verified first, checksum + SHA-256 recomputed and re-verified, `revert` restores byte for byte, other firmware refused (`tests/test_stockpatch.py`, `docs/lua-apps.md`) |
| `tools/xtapp.py new NAME DIR` / `pack APP_DIR` / `info F.xtapp` / `install SD.img APP_DIR` | Lua apps for the stock: `app.xtapp` = a 0x80-byte XTAP header + the manifest JSON, unsigned; the manifest is validated as the firmware does (`permissions` only `sys.battery`/`sys.status`, else the stock drops the app silently); `install` puts the app under `/XTApps/<app_id>/` on a card image; `new` scaffolds an `index.lua` documenting the confirmed API (`g:*` drawing on the 480×800 portrait frame, `ctx.*` services, the callbacks; `print` reaches nothing) (`tests/test_xtapp.py`; `tests/test_lua_apps.py` boots the patched stock and runs the hello app) |
...
2. Never `erase-flash`, never `espefuse burn-*`, never write 0x0..0x10000 (bootloader, partition
   ...
   `images/device/stock-app1-7.5.4.bin`), otadata entry 1 seq 2 → **app1 boots** (`tools/device.py slots`).
   `flash-crosspoint` / `restore-stock` read otadata and refuse to write app0 while app1 boots; `--slot app1`
   is the flag that would write the booting slot, and no tool here writes otadata.
...
`image-sd`, `slots [--from-dump PATH]`, `flash-crosspoint [--firmware F] [--preserve-stock] [--slot S] --yes`,
`restore-stock [--slot S] --yes`. The partition table and otadata are parsed, never assumed:
`slots` prints the table, which slot the bootloader starts and what each app slot holds; the two
writers read otadata first and refuse before writing when their target is not the booting slot.
Every write-capable command prints its plan and re-reads the device before writing.
`_refuse_forbidden_write` is a structural guard on every esptool command the file builds: no
`erase-*`, and no `write-flash` below 0x10000, so nothing here can move the boot pointer.
