...
| `tools/xtfont.py dump IN.xtf OUT.png [--text T\|--glyph CP]` / `build TTF --id ID --name N OUT_DIR` / `install SD.img PKG.xtfont [--direct]` | the stock's `.xtf` v2 system-font packages: parse and render (metrics `advance_x, advance_y, x_off, y_off`, pinned 0 px against the Home title), rasterise a TTF into the 20/24 px cells with manifest, `preview.xic` and the `.xtfont` ZIP, put it on a card (`--direct` rewrites `selection.config`); no `.hot.xtfp` needed, but a generated font is still refused by 7.2.4 at load — the header hash at 0x30 is under investigation (`tests/test_xtfont.py`) |
