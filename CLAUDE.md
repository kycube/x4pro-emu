...
tools/         x4emu shim, x4emu_mcp.py, mkflash.py, mksd.py, mkefuse.py, nvsedit.py, xic2png.py, stockdev.py, stockpatch.py, stockstrings.py, xtfont.py, xtapp.py, ghidra_stock.py (+ ghidra/), device.py, epdtrace.py, appdis.py, tcbwalk.py
...
| `tools/ghidra_stock.py analyze [--force]` / `xrefs ADDR` / `callers ADDR` / `func ADDR` / `decompile ADDR` | Ghidra over the stock 7.2.4 app image (Ghidra 12.1.3 + openjdk@21 from Homebrew; Ghidra ships the Xtensa processor): `analyze` (~1 min, idempotent) imports the image raw as `Xtensa:LE:32`, rebuilds one memory block per ESP segment at its load VA, labels the ROM from `images/rom/esp32s3_rev0_rom.nm`, exports `functions/xrefs/strings/calls.txt` to `images/ghidra/stock-7.2.4/` (24.8 k functions, 435 k xrefs, 96.5 k calls, 9.8 k strings); the queries are pure Python over those files (0.1 s), `decompile` a headless decompile of one function (~3 s; the SLEIGH misses a few opcodes — `appdis.py` is the fallback). GUI: `$(brew --prefix ghidra)/libexec/ghidraRun` on `images/ghidra/stock-7.2.4.gpr`, closed before `analyze`/`decompile` |
| `tools/stockpatch.py list IMAGE` / `apply\|revert IMAGE PATCH... [--check] [-o OUT]` | named byte patches on a stock 7.2.4 image, bare app or 16 MB flash (app0 at 0x10000): `developer-menu` (= stockdev.py), `hidden-menu-rows` (Preload List / Statistics), `lua-apps-row` (a sixth nav row "Lua Apps" → the app list); each site's bytes verified first, checksum + SHA-256 recomputed and re-verified, `revert` restores byte for byte, other firmware refused (`tests/test_stockpatch.py`, `docs/lua-apps.md`) |
